var RTree = function(width){
	...
	this.getxyMax = function() {
		return xyMAX;
	};
	
	this.getxyMin = function() {
		return xyMIN;
	};
		...
		do {
			...
			for(var i = nodes.length-1; i >= 0; i--) {
			  ...
			  if(RTree.Rectangle.overlap_rectangle(rect, ltree)) {
                  //console.log("hay overlap " ,MOIU.x ," " , MOIU.y);
    ...
    this.getAlertCh = function() {
        //alert("cambio alert ch");
		return AlertCharge;
	};
	
	this.setAlertCh = function() {
        //alert("cambio alert ch");
		AlertCharge = true;
	};
    ...
    this.treeSearch = function(x,y,w,h){
        
        //alert("OK");
        var results=this.search({x:x,y:y,w:w,h:h});
        RRES = results;
        console.log(RRES);
        if(results.length==0)
            return [];
